...
interface SubscriptionPageContentProps {
    initialData: SubscriptionPageData;
}

/**
 * Client Component: Handles subscription interactivity
 * - Stripe sync
 * - Cancellation/reactivation
 * - Dev mode plan switching
 */
export function SubscriptionPageContent({ initialData }: SubscriptionPageContentProps) {
    ...
    const [subscription, setSubscription] = useState<SubscriptionData>(initialData.subscription);
    const [usage, setUsage] = useState<UsageData>(initialData.usage);
    const devMode = initialData.devMode;
    ...
    return (
        <div className="space-y-8">
            <div className="flex items-start justify-between">
                ...
                {!devMode && subscription?.stripe_subscription_id && (
                    <button
            ...
            {devMode && (
                <div className="bg-yellow-50 dark:bg-yellow-900/20 border-2 border-yellow-300 dark:border-yellow-700 border-dashed rounded-xl p-6">
                    <div className="flex items-center gap-2 mb-4">
                        <Wrench className="w-5 h-5 text-yellow-600 dark:text-yellow-400" />
                        <h3 className="text-lg font-semibold text-yellow-800 dark:text-yellow-300">Dev Mode: Plan Switcher</h3>
                    </div>
            ...
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-xl p-6 border border-indigo-100 dark:border-indigo-800">
                <div className="flex items-start justify-between">
                    ...
                    {subscription?.plan !== 'agency' && !devMode && (
                        <Link
                            href="/pricing"
